...
def build_list_page(record_file, page_title, active, heading):
    # Robust reader: the media record carries a leading HTML/reference block and
    # baked-in "## YYYY" year subheaders; the talks record is flat. Start at the
    # single-hash H1 title, take its bracket count, then collect every citation
    # line after it, skipping year subheaders, blanks, rules, and stray notes.
    lines = open(record_file, encoding="utf-8").read().split("\n")
    count, entries, started = None, [], False
    for ln in lines:
        s = ln.strip()
        if not started:
            m = re.match(r'^#\s+(.*)$', s)
